...
describe('Task 1 family homepage foundation', () => {
  ...
  it('uses the silent timelapse hero and keeps the homepage ordered by family intent', () => {
    ...
    for (const section of ['<TimelapseHero />', '<TrustStrip />', '<FamiliesIntro />', '<NextEventSection', '<EventProof />', '<MissionSection />', '<BranchesSection />', '<FinanceSection />', '<ContactCta />']) {
      expect(page).toContain(section)
    }
    const orderedSections = ['<TimelapseHero />', '<TrustStrip />', '<FamiliesIntro />', '<NextEventSection', '<EventProof />', '<MissionSection />', '<BranchesSection />', '<FinanceSection />', '<ContactCta />']
  ...
  it('keeps locked public navigation and protected-route omission', () => {
    ...
    for (const href of ['/events', '/volunteer', '/contact', '/fundraiser', '/wishlist', '/newsletter', '/faq']) {
