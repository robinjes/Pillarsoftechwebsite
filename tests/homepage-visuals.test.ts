...
describe('image-led homepage proof surfaces', () => {
  ...
  it('keeps real event photography and a bounded hero scale', () => {
    ...
    expect(new Set(imagePaths).size).toBeGreaterThanOrEqual(7)
    expect(page).toContain('lg:text-[4.35rem]')
    expect(page).not.toContain('text-[5.8rem]')
    expect(page).not.toContain('text-[7.5rem]')
    expect(page).toContain('Explore events')
    expect(page).toContain('Volunteer')
    for (const retiredDetail of ['Choose a problem.', 'Make a prototype.', 'Try it, notice, adjust.', 'Teach it forward.']) {
      expect(page).not.toContain(retiredDetail)
    }
    ...
    expect(heroVisual).toContain('reducedMotion ? 0 : y')
    expect(heroVisual).not.toContain('data-hero-contact-sheet')
    ...
    expect(heroVisual).not.toContain('drive-04.webp')
  })

  it('keeps the public shell free of the retired atmosphere and crop decoration', () => {
    ...
    expect(styles).not.toContain('public-atmosphere')
    expect(styles).not.toContain('registration-mark')
    expect(styles).not.toContain('cut-mark')
    expect(styles).not.toContain('radial-gradient')
  })

  it('keeps proof captions, audience roles, and unobtrusive assembly cues content-bearing', () => {
    const eventProof = readSource('components/site/EventProof.tsx')
    const eventProofPhoto = readSource('components/site/EventProofPhoto.tsx')
    ...
    expect(eventProofPhoto).toContain('<figcaption>{caption}</figcaption>')
    expect(eventProofPhoto).not.toContain('Field note')
    ...
    expect(assembly).toContain('workshop-stage-list')
    expect(assembly).toContain('workshop-stage-list__marker')
    expect(assembly).toContain('lg:text-5xl')
    expect(assembly).not.toContain('lg:text-6xl')
    expect(assembly).toContain('Assembled rover')
    expect(assembly).not.toContain('workshop-registration-layer')
    expect(assembly).not.toContain('ROVER READY')
    expect(assembly).not.toContain('radial-gradient')
    expect(styles).toContain('.workshop-stage-list__marker')
    expect(styles).toContain('.workshop-stage-list__item[data-active=\'true\']')
    expect(styles).not.toContain('workshop-stage-readout')
  })
})
