...
describe('editorial homepage foundation', () => {
  ...
  it('keeps the workshop assembly as a lazy, responsive, object-only WebGL scene', () => {
    ...
    expect(desktopWorkshop).toContain('Assembled rover')
    expect(desktopWorkshop).toContain("currentStage.isReady ? 'Assembled rover'")
    expect(desktopWorkshop).toContain('in progress')
    expect(desktopWorkshop).toContain('workshop-stage-list')
    expect(desktopWorkshop).not.toContain('workshop-stage-readout')
    expect(desktopWorkshop).not.toContain('workshop-registration-layer')
  ...
  it('locks the palette and typography tokens', () => {
    ...
    for (const token of ['#F5F1E8', '#0D1117', '#0A1B33', '#B9DCF1', '#2A65B7', '#FFFDF8']) {
  ...
  it('keeps header navigation direct, keyboard-complete, and inclusive of FAQ support', () => {
    ...
    expect(navbar).not.toContain('Find an event')
