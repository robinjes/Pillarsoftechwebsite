...
describe('editorial homepage foundation', () => {
  ...
  it('keeps the workshop assembly as a lazy, responsive, object-only WebGL scene', () => {
    ...
    expect(desktopWorkshop).toContain('RoundedBoxGeometry')
    ...
    expect(desktopWorkshop).toContain('caseRootRef')
    expect(desktopWorkshop).toContain('roverRootRef')
    expect(desktopWorkshop).toContain('CASE_EXIT')
    ...
    for (const primitive of ['boxGeometry', 'cylinderGeometry', 'meshStandardMaterial', 'planeGeometry']) {
      expect(desktopWorkshop).toContain(primitive)
    }
    for (const part of ['chassisRef', 'wheelFrontLeftRef', 'wheelFrontRightRef', 'wheelRearLeftRef', 'wheelRearRightRef', 'boardRef', 'batteryRef', 'sensorRef']) {
      expect(desktopWorkshop).toContain(part)
    }
