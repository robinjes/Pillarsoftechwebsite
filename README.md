...
Visitor chat ownership uses a fresh 32-byte opaque token in a Secure, HttpOnly, SameSite=Lax cookie scoped to `/api/chat`. The server stores only the HMAC-SHA256 digest made with `CHAT_TOKEN_PEPPER`; names, emails, message bodies, and raw tokens are not placed in browser storage, URLs, logs, or error responses. Expired or invalid ownership fails closed, and chat messages remain bounded plain text.
